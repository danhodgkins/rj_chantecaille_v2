...
namespace Recorder
{
    ...
    {
        private const float SCREEN_WIDTH = 720f; //480f; // 720f / 2;
    ...
    private AndroidJavaObject androidRecorder;
        ...
        using (AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
        {
            ...
            if (Input.deviceOrientation == DeviceOrientation.Portrait || Input.deviceOrientation == DeviceOrientation.PortraitUpsideDown || Input.deviceOrientation == DeviceOrientation.FaceDown || Input.deviceOrientation == DeviceOrientation.FaceUp || Input.deviceOrientation == DeviceOrientation.Unknown)
                androidRecorder.Call("setupVideo", width, height,(int)(1f * width * height / 100 * 240 * 7), 30);
            else
                androidRecorder.Call("setupVideo", height, width, (int)(1f * width * height / 100 * 240 * 7), 30);
    ...
    androidRecorder.Call("cleanUpRecorder");
        ...
        public void PrepareRecorder()
        {
            ...
            int height = 1;
                ...
                {
                    height = (int)(Screen.height > SCREEN_WIDTH ? SCREEN_WIDTH : Screen.height);
                    width = Screen.height > SCREEN_WIDTH ? (int)(Screen.width * SCREEN_WIDTH / Screen.height) : Screen.width;
                } else
                {
                    width = (int)(Screen.width > SCREEN_WIDTH ? SCREEN_WIDTH : Screen.width);
                    height = Screen.width > SCREEN_WIDTH ? (int)(Screen.height * SCREEN_WIDTH / Screen.width) : Screen.height;
                }
            if (width % 2 != 0){
                width --;
            }
        ...
        public void VideoRecorderCallback(string message)
        {
            switch (message)
            {
                case "init_record_error":
                    ShowToast("init_record_error");
                    break;
                case "init_record_success":
                    ShowToast("init_record_success");
                    break;
                case "start_record":
                    ShowToast("start_record");
                    break;
                case "stop_record":
                    ShowToast("stop_record");
                    break;
            }
        }
